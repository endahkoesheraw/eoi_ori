...
class Create_eoi_form extends CI_Controller {
	...
	function __construct(){
		...
		if ($this->session->userdata('id')) {
			...
			$a=$this->model_eoiform->getEoiByUid($id)->num_rows();
			if($a>=1){
				 redirect('eoi_form');
				}
		}
	...
	function delete($user_id){
		...
		$this->model_eoiform->delete_eoiform($user_id);
